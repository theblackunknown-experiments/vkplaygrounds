...
Presentation::Presentation(const blk::Engine& vkengine, const blk::Surface& vksurface, const VkExtent2D& resolution)
	: mEngine(vkengine)
	, mSurface(vksurface)
...
VkExtent2D Presentation::recreate_swapchain()
{
	...
	{ // Creation
		...
		auto finder_composite_alpha =
			std::ranges::find_if(kPreferredCompositeAlphaFlags, [&capabilities](const VkCompositeAlphaFlagBitsKHR& f) {
		...
		{ // Cleaning
			if (!mCommandBuffers.empty())
				vkFreeCommandBuffers(
					mDevice, mCommandPool, static_cast<std::uint32_t>(mCommandBuffers.size()), mCommandBuffers.data());
...
blk::Presentation::Image Presentation::acquire_next(std::uint64_t timeout)
{
	...
	const VkResult status =
		vkAcquireNextImageKHR(mDevice, mSwapchain, timeout, mAcquiredSemaphore, VK_NULL_HANDLE, &index);
	CHECK(status);
	return Image{
		index,
