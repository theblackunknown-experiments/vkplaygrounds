	...
	, mStagingBuffer(kStagingBufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT)
{
	{ // Device
		...
		const std::size_t queue_count = std::accumulate(
			std::begin(info_queues),
			std::end(info_queues),
			std::size_t{0},
			[](std::size_t current, const VkDeviceQueueCreateInfo& info) { return current + info.queueCount; });
		...
		{
			auto finder =
				std::ranges::find(mPhysicalDevice.mQueueFamilies, info.queueFamilyIndex, &blk::QueueFamily::mIndex);
	...
	{     // Staging
		...
		// Memory
		auto memory_type = vkphysicaldevice.mMemories.find_compatible(
