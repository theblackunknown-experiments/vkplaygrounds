...
int main(int argc, char* argv[])
{
	...
	const std::span<std::uint32_t> shadercode(
		reinterpret_cast<std::uint32_t*>(buffer.data()), buffer.size() / sizeof(std::uint32_t));
...
#include <array>
	...
	ostream << "constexpr const std::array<std::uint32_t, " << shadercode.size() << "> " << variable << " {"
			<< std::endl;
