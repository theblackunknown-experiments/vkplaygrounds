...
struct MemoryHeap
{
	...
	constexpr bool supports(VkMemoryHeapFlags flags) const { return mHeap.flags & flags; }

	constexpr operator VkMemoryHeap() const { return mHeap; }
};
...
struct Memory
{
	...
	constexpr operator VkDeviceMemory() const { return mMemory; }

	CORE_EXPORT void* map(const Buffer& buffer) const;
	CORE_EXPORT void unmap(const Buffer& buffer) const;
};
