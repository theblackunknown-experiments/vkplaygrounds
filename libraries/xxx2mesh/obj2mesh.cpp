...
result_t wrap_as_buffers(const obj_t& obj)
{
	...
	static_assert(std::is_same_v<decltype(obj.vertices)::value_type, blk::meshes::obj::v1::vertex_t>);
	result.vertices.pointer = obj.vertices.data();
	result.vertices.count = obj.vertices.size();
	result.vertices.stride = sizeof(blk::meshes::obj::v1::vertex_t);
	...
	static_assert(std::is_same_v<decltype(obj.points)::value_type, blk::meshes::obj::v1::point_t>);
	result.points.pointer = obj.points.data();
	result.points.count = obj.points.size();
	result.points.stride = sizeof(blk::meshes::obj::v1::point_t);
	...
	if (index_texcoord)
	{
		static_assert(std::is_same_v<decltype(obj.texcoords)::value_type, blk::meshes::obj::v1::texcoord_t>);
		result.texcoords = blk::BufferCPU{
			.pointer = obj.texcoords.data(),
			.count = obj.texcoords.size(),
			.stride = sizeof(blk::meshes::obj::v1::texcoord_t),
	...
	if (index_normal)
	{
		static_assert(std::is_same_v<decltype(obj.normals)::value_type, blk::meshes::obj::v1::normal_t>);
		result.normals = blk::BufferCPU{
			.pointer = obj.normals.data(),
			.count = obj.normals.size(),
			.stride = sizeof(blk::meshes::obj::v1::normal_t),
			.element_stride = sizeof(float),
	...
	for (const group_t& group : obj.groups)
	{
		...
		static_assert(std::is_same_v<decltype(group.faces)::value_type, blk::meshes::obj::v1::face_t>);
		result.faces_by_group.emplace(
			group.name,
			blk::BufferCPU{
				.pointer = group.faces.data(),
				.count = group.faces.size(),
				.stride = sizeof(blk::meshes::obj::v1::face_t),
