...
inline namespace v0
{
OBJ2MESH_EXPORT CPUMesh load(const fs::path& path);
}
...
inline bool is_face_attribute_separator(char c)
{
	return (c == '/');
}

inline bool is_space_separator(char c)
{
	return (c == ' ') || (c == '\t');
}

inline bool is_token_separator(char c)
{
	return is_space_separator(c) || (c == '\n');
}
...
template<typename Iterator>
std::tuple<Iterator, std::string, face_attribute_t>
parse_face_attributes(Iterator iterator, Iterator last, face_attribute_t attribute)
{
	...
	for (; (iterator != last); ++iterator)
	{
		...
		if (is_face_attribute_separator(character))
		{
			...
			switch (attribute)
			{
			case face_attribute_t::Vertex: next_attribute = face_attribute_t::TexCoord; break;
			case face_attribute_t::TexCoord: next_attribute = face_attribute_t::Normal; break;
			case face_attribute_t::Normal: next_attribute = face_attribute_t::Vertex; break;
			default: throw std::runtime_error("Unexpected face attribute: " + std::to_string((int)attribute));
			}
			break;
		}
	...
	for (; iterator != last;)
	{
		...
		case 'f': {
			...
			for (; !parsed && (iterator != last);)
			{
				...
				break;
				default: {
					std::tie(iterator, token, next_attribute) =
						parse_face_attributes(iterator, last, expected_attribute);
